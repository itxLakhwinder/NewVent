...
class Jwt
{
    ...
    public static function sendPushNotification($msg, $dev_token){

        $authKey = public_path()."/AuthKey_Q9QQ5RJ454.p8";
        $arParam['teamId'] = 'FMM2HHY9S8';// Get it from Apple Developer's page
        $arParam['authKeyId'] = 'Q9QQ5RJ454';
        $arParam['apns-topic'] = 'com.ventspace.dev';
        $arClaim = ['iss'=>$arParam['teamId'], 'iat'=>time()];
        $arParam['p_key'] = file_get_contents($authKey);        
        $arParam['header_jwt'] = static::encode($arClaim, $arParam['p_key'], $arParam['authKeyId'], 'RS256');

        // Sending a request to APNS
        $stat = static::push_to_apns($arParam, $msg, $dev_token);
        return $stat;
        
    }

    public static function push_to_apns($arParam, $ar_msg, $dev_token){

        $arSendData = array();

        $url_cnt = url('/');
        $arSendData['aps']['alert']['title'] = sprintf($ar_msg["title"]); // Notification title
        $arSendData['aps']['alert']['body'] = sprintf($ar_msg["text"]); // body text
