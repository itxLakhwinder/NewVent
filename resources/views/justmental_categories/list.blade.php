...
<section class="wrapper" id="vueEl">
  <div class="row">
    <div class="col-lg-12">
      <h3 class="page-header"><i class="fa fa fa-bars"></i> Just Mental Podcast Categories ({{count($topics)}})</h3>
    </div>
  </div>
  @if (session('success'))
    ...
    <tbody class="sort_menu">
      <tr v-for="item in items" :data-id="item.id">
        ...
        <td>
          ...
          <a href="#" v-bind:href="'justmental_categories/delete/' + item.id" class="text-danger" onclick="return confirm('Are you sure?');">Delete</a>
        </td>
      </tr>
    </tbody>
  </table>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="addTopicLabel">ADD JUST MENTAL PODCAST CATEGORIES</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <form action="{{route('justmental_categories.add')}}" method="POST">
          @csrf
  ...
  <div class="modal fade" id="updateTopics" tabindex="-1" role="dialog" aria-labelledby="updateQuesLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form action="{{route('justmental_categories.update')}}" method="POST">
          @csrf
          <div class="modal-body">
            <div class="form-group">
...
<script>
  $(document).ready(function(){
    ...
    function updateToDatabase(idString){
       ...
       $.ajax({
            url:'{{url("/indexing_justmental_categories")}}',
            method:'POST',
            data:{ids:idString},
            success:function(){
