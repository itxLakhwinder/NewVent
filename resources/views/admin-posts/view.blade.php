...
<section class="wrapper">
    ...
    <tbody>
      ...
      <tr>
        <th>category</th>
        <td>{{@$topic->available_topic}}</td>
      </tr> 
      <tr>
        <th>group</th>
        <td>{{@$topic->category_group}}</td>
      </tr> 
      <tr>
        <th>created_at</th>
         <td>{{ date('m-d-Y H:i', strtotime(@$topic->created_at)) }}</td>          
      </tr>     
    </tbody>
    </table>
    <h3>
      ...
      <tbody>
        @if(count($topic->comments))
          ...
          <tr>
            ...
            <td>{{ date('m-d-Y H:i', strtotime(@$comments->created_at)) }}</td>
          </tr>
          @endforeach
        @endif
      </tbody>
