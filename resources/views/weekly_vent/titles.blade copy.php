  ...
  <table class="table table-bordered panel" id="dataTable" width="100%" cellspacing="0">
    ...
    <tbody >
      <tr v-for="data in dataItems">
        ...
        <td class="test1">@{{data.group.name}}</td>
        <td class="test1">@{{data.type}}</td>
        <td class="test1">@{{data.category.title}}</td>
        ...
        <td>
          ...
          <a href="#" v-bind:href="'/weekly-vent/titles/delete/' + data.id" class="text-danger" onclick="return confirm('Are you sure?');">Delete</a>
        </td>
      </tr>
    </tbody>
  </table>
        ...
        <form action="{{route('weekly_vent.title.add')}}" method="POST">
          ...
          <div class="modal-body">
            ...
            <div class="form-group">
              ...
              <select class="form-control" id="type" name="type" v-model="type" required @change="selectType()">
                  <option value="">Select</option>
          ...
          <div class="form-group">
            ...
            <input type="date" class="form-control" id="valid_from" name="valid_from" :min="CurrentDate" required v-model="validFromDate">
          </div>
          <div class="form-group">
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form action="{{route('weekly-vent.title.update')}}" method="POST">
          ...
          <div class="form-group">
              <label for="group">Group:</label>
              <select class="form-control" id="group" name="group_id" v-model="item.group_id"  required>
                  <option value="">Select</option>
                <option v-for="group in groups" :value="group.id">@{{group.name}}</option>
                </select>
          </div> 
          <div class="form-group">
            ...
            <select class="form-control" id="type" name="type" v-model="item.type"  @change="selectTypeForUpdate()" required >
                <option value="">Select</option>
              <option value="adults">Adults</option>
              <option value="teens">Teens</option>
            </select>
        ...
        <div class="form-group">
          <label for="category">Category:</label>
          <select class="form-control" id="category" name="category_id" v-model="categoryid" required>
            <option value="">Select</option>
          <option v-for="category in categories" :value="category.id">@{{category.title}}</option>
          </select>
        </div>
        <div class="form-group">
          <label for="valid_from">Valid From Date:</label>
          <input type="date" class="form-control" id="valid_from" name="valid_from" v-model="item.valid_from" :min="CurrentDate" required>
        </div>
        <div class="form-group">
          <label for="valid_to">Valid Till Date:</label>
          <input type="date" class="form-control" id="valid_to" name="valid_to" v-model="item.valid_to" :min="item.valid_from" required>
        </div> 
          <div class="form-group">
            <label for="pwd">Description:</label>
        ...
        methods: {
          editQues: function(item) {
            ...
            this.categories = this.AllCategories.filter((el) => {
                return el.type.includes(this.item.type);
            }); 
          ...
          selectType(){
            this.categories = this.AllCategories.filter((el) => {
                return el.type.includes(this.type);
            });
          },
          selectTypeForUpdate(){
            this.categories = this.AllCategories.filter((el) => {
