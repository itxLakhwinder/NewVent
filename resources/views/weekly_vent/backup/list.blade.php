...
<section class="wrapper" id="vueEl">
  <div class="row">
    <div class="col-lg-12">
      <h3 class="page-header"><i class="fa fa fa-bars"></i> Weekly Vent Questions ({{count($weekly_vent_questions)}})</h3>
    </div>
  </div>
  @if (session('success'))
    ...
    <tbody>
      <tr v-for="item in items">
        ...
        <td>@{{timeFormat(item.created_at)}}</td>
         <td>
          <a v-if="item.status == '1'" v-bind:href="'/weekly-vent/question/enable/' + item.id" onclick="return confirm('Are you sure?');">Enable</a>
          <a v-if="item.status != '1'" v-bind:href="'/weekly-vent/question/disable/' + item.id" onclick="return confirm('Are you sure?');">Disable</a>
        </td>
        <td>
          <a href="javascript:void(0)" class="" @click="editQues(item)">Edit</a> |
          <a href="#" v-bind:href="'/weekly-vent/question/delete/' + item.id" class="text-danger" onclick="return confirm('Are you sure?');">Delete</a>
        </td>
      </tr>
    </tbody>
  </table>
  ...
  <div class="modal fade" id="addQues" tabindex="-1" role="dialog" aria-labelledby="addQuesLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form action="{{route('weekly_vent.question.add')}}" method="POST">
          @csrf
  ...
  <div class="modal fade" id="updateQues" tabindex="-1" role="dialog" aria-labelledby="updateQuesLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <form action="{{route('weekly_vent.question.update')}}" method="POST">
          @csrf
          <div class="modal-body">
            <div class="form-group">
                <label for="group">Title:</label>
                <select class="form-control" id="title" name="title_id" v-model="item.title_id"  required>
                    <option value="">Select</option>
                  <option v-for="title in titles" :value="title.id">@{{title.title}}</option>
                  </select>
            </div> 
...
<script type="text/javascript">
  ...
  const vApp = new Vue({
        ...
        data: {            
            item: {},           
            items: <?= json_encode($weekly_vent_questions) ?>,
